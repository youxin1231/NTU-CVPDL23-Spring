...
class Trainer:
    ...
    def eval_and_save(self):
        ...
        if self.main_process:
            ...
            mAP_path = os.path.join(self.save_dir, 'mAP50.txt')
            ...
            if self.epoch == self.max_epoch // 3:
                save_checkpoint(ckpt, False, save_ckpt_dir, model_name=f'1')
            if self.epoch == self.max_epoch // 3 * 2:
                save_checkpoint(ckpt, False, save_ckpt_dir, model_name=f'2')
            if self.epoch == self.max_epoch - 1:
                save_checkpoint(ckpt, False, save_ckpt_dir, model_name=f'3')
