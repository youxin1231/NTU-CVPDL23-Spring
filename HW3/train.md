...
### Download datasets and convert to YOLO formats.
```
if [ ! -d datasets ]; then
...
### Copy source model checkpoint.
```
cp runs/train/0/weights/best_ckpt.pt weights/0.pt
cd ..
```

### Prepare self training datasets.
```
if [ ! -d datasets/org_with_fog ]; then
    ...
    # Inference on fog dataset to create fake labels.
    ...
    # Copy data into org_with_fog datasets.
...
```
### Self training on org_with_fog datasets.
